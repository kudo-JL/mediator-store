...
// Always use Amiri for any text that contains Arabic characters,
// since Helvetica/Times have no Arabic glyphs and would render them as garbage.
const FONT_REG = useArabic ? 'Amiri' : 'Helvetica';
const FONT_BOLD = useArabic ? 'Amiri' : 'Helvetica-Bold';

function ar(text) {
  if (!text) return '';
  // Try multiple API shapes used by different versions of the package
...
async function streamInvoice({ db, orderId, order, items, settings, verifyUrl, outStream }) {
  ...
  // ---- Invoice meta (Latin + Arabic, uses Amiri) ----
...
// Use company_* first, fall back to contact_* if company_* is empty.
...
async function generateInvoiceForOrder(db, orderId) {
  ...
  // 1) If this order already has an invoice, REUSE its invoice_number.
  //    Generate only a fresh PDF (so admin always gets the latest status).
  const existing = db.prepare('SELECT * FROM invoices WHERE order_id = ?').get(orderId);
  const reuseNumber = existing ? existing.invoice_number : null;

  // 2) Generate the PDF (always fresh — reflects current order status).
  ...
  // 3) Save to DB — explicit INSERT or UPDATE (no UPSERT ambiguity).
  if (reuseNumber) {
    // Order already has an invoice — just update the pdf_path.
    db.prepare(
      `UPDATE invoices
       SET pdf_path = ?, generated_at = CURRENT_TIMESTAMP
       WHERE order_id = ?`,
    ).run(finalPath, orderId);
  } else {
