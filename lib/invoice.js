...
async function streamInvoice({ db, orderId, order, items, settings, verifyUrl, outStream }) {
  ...
  doc.fontSize(9).fillColor(muted).font('Helvetica')
    .text(settings.site_tagline || 'منصة الوساطة للبيع الإلكتروني - المغرب', { align: 'right' });
  doc.moveDown(0.6);
  doc.strokeColor(accent).lineWidth(1.5).moveTo(50, doc.y).lineTo(545, doc.y).stroke();
  doc.moveDown(0.6);

  // Frozen date: use order.created_at so re-downloads show the same date.
  ...
  const colY = doc.y;
  doc.fontSize(11).fillColor(accent).font('Helvetica-Bold').text('Vendeur / البائع', { align: 'right' });
  doc.fontSize(9).fillColor(ink).font('Helvetica');
  doc.text(settings.site_name || 'Mediator Store', { align: 'right' });
  if (settings.company_address) doc.text(settings.company_address, { align: 'right' });
  if (settings.company_city) doc.text(settings.company_city, { align: 'right' });
  ...
  doc.fontSize(11).fillColor(accent).font('Helvetica-Bold').text('Client / الزبون', 50, colY);
  doc.fontSize(9).fillColor(ink).font('Helvetica');
  ...
  if (order.customer_notes) {
    doc.moveDown(2);
    doc.fontSize(10).fillColor(accent).font('Helvetica-Bold').text('Notes / ملاحظات');
    doc.fontSize(9).fillColor(ink).font('Helvetica').text(order.customer_notes, { width: 495 });
  }
  ...
  doc.fontSize(7).fillColor(muted).text(
    `Vérifier l'authenticité / للتحقق من صحة الفاتورة: ${qrUrl}`,
